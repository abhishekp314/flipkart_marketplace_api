/*
 * Flipkart Marketplace API
 * This API is offered by Flipkart Marketplace to sellers and partners for orders and listing management. <br /><br />You will need an access token to explore our APIs in the sandbox environment. For details on getting an access token, refer to the <a href='https://seller.flipkart.com/api-docs/FMSAPI.html#sandbox-environment'> documentation</a>. <br />Once you get the access token, use it to authorize the requests using the  <code>Authorize</code> button below. e.g. if your access token is <code>ACCTOK1</code>, the input for authorization should be <code>Bearer ACCTOK1</code>
 *
 * OpenAPI spec version: 3.0
 * Contact: seller-api-queries@flipkart.com
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 *
 * Swagger Codegen version: 3.0.35
 *
 * Do not edit the class manually.
 *
 */
(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', '../../src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require('../../src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.FlipkartMarketplaceApi);
  }
}(this, function(expect, FlipkartMarketplaceApi) {
  'use strict';

  var instance;

  describe('(package)', function() {
    describe('DispatchRequestV3', function() {
      beforeEach(function() {
        instance = new FlipkartMarketplaceApi.DispatchRequestV3();
      });

      it('should create an instance of DispatchRequestV3', function() {
        // TODO: update the code to test DispatchRequestV3
        expect(instance).to.be.a(FlipkartMarketplaceApi.DispatchRequestV3);
      });

      it('should have the property shipmentId (base name: "shipmentId")', function() {
        // TODO: update the code to test the property shipmentId
        expect(instance).to.have.property('shipmentId');
        // expect(instance.shipmentId).to.be(expectedValueLiteral);
      });

      it('should have the property tentativeDeliveryDate (base name: "tentativeDeliveryDate")', function() {
        // TODO: update the code to test the property tentativeDeliveryDate
        expect(instance).to.have.property('tentativeDeliveryDate');
        // expect(instance.tentativeDeliveryDate).to.be(expectedValueLiteral);
      });

      it('should have the property dispatchDate (base name: "dispatchDate")', function() {
        // TODO: update the code to test the property dispatchDate
        expect(instance).to.have.property('dispatchDate');
        // expect(instance.dispatchDate).to.be(expectedValueLiteral);
      });

      it('should have the property deliveryPartner (base name: "deliveryPartner")', function() {
        // TODO: update the code to test the property deliveryPartner
        expect(instance).to.have.property('deliveryPartner');
        // expect(instance.deliveryPartner).to.be(expectedValueLiteral);
      });

      it('should have the property trackingId (base name: "trackingId")', function() {
        // TODO: update the code to test the property trackingId
        expect(instance).to.have.property('trackingId');
        // expect(instance.trackingId).to.be(expectedValueLiteral);
      });

      it('should have the property locationId (base name: "locationId")', function() {
        // TODO: update the code to test the property locationId
        expect(instance).to.have.property('locationId');
        // expect(instance.locationId).to.be(expectedValueLiteral);
      });

      it('should have the property invoice (base name: "invoice")', function() {
        // TODO: update the code to test the property invoice
        expect(instance).to.have.property('invoice');
        // expect(instance.invoice).to.be(expectedValueLiteral);
      });

      it('should have the property dispatchLocation (base name: "dispatchLocation")', function() {
        // TODO: update the code to test the property dispatchLocation
        expect(instance).to.have.property('dispatchLocation');
        // expect(instance.dispatchLocation).to.be(expectedValueLiteral);
      });

    });
  });

}));
